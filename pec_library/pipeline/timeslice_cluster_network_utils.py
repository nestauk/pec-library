...
def get_subgraph_clusters(G_timeslice):
    subgraph_cluster = [node[1] for node in G_timeslice.nodes(data="timeslice cluster number")]
    return [node[0] for node in Counter(subgraph_cluster).most_common()]

def get_subgraph_cluster_nodes(G_timeslice, cluster):
    return [node for node, node_info in G_timeslice.nodes(data=True) if node_info["timeslice cluster number"] == cluster]
...
def add_cluster_names(subgraph_communities:dict):
    ...
    for subgraph in subgraph_communities.values():
        ...
        for community in communities:
            node_names = [y['_nx_name'] for x,y in subgraph.nodes(data=True) if y['timeslice cluster number'] == community]
            community_names[community] = '-'.join(get_tfidf_top_features(node_names, 3))

    for subgraph in subgraph_communities.values():
        node_cluster_names = dict(zip(list(subgraph.nodes()),
                                        [community_names[subgraph.nodes[n]['timeslice cluster number']] for n in subgraph.nodes]))
...
def add_cluster_colors(subgraph_communities:dict):
    ...
    """
    cluster_numbers = [list(set([node_info['timeslice cluster number'] for node, node_info in subgraph.nodes(data=True)])) for subgraph in subgraph_communities.values()]
    all_clusters = list(set(itertools.chain(*cluster_numbers)))
    hex_colors = ["#%06x" % random.randint(
            0, 0xFFFFFF) for _ in range(len(all_clusters))]
    cluster_colors = dict(zip(all_clusters, hex_colors))

    for subgraph in subgraph_communities.values():
        node_cluster_colors = dict(zip(list(subgraph.nodes()),
                                       [cluster_colors[subgraph.nodes[n]['timeslice cluster number']] for n in subgraph.nodes]))
...
def timeslice_subject_pair_coo_graph(G, timeslice_interval: int) -> Dict:
    ...
    min_timeslice, max_timeslice = min(pairs_first_published) + timeslice_interval, max(pairs_first_published) + timeslice_interval
    
    G_timeslices = dict()
    for i, timeslice in enumerate(range(min_timeslice, max_timeslice, timeslice_interval)):
        subgraph_edges = [(u,v) for u,v,e in G.edges(data=True) if e["first_published"] <= timeslice]
...
def cluster_timeslice_subject_pair_coo_graph(G_timeslices: dict):
    ...
    for timeslice, subgraph in G_timeslices.items():
        ...
        partitions = la.find_partition(subgraph_igraph, 
    ...
    for timestamp, subgraph_community in subgraph_communities.items():
        for node in subgraph_community.nodes(data=True):
            node[1]['timeslice cluster number'] =  timestamp + '_' + str(node[1]['cluster number'])
...
def sanitise_clusters(timeslice_x, timeslice_y):
    ...
    """
    subgraph_clusters = [get_subgraph_clusters(subgraph) for subgraph in (timeslice_x, timeslice_y)]
    cluster_perms = list(itertools.product(subgraph_clusters[0], subgraph_clusters[1]))

    perm_dists = []
    for cluster_perm in cluster_perms:
        timeslice_x_nodes, timeslice_y_nodes = get_subgraph_cluster_nodes(timeslice_x, cluster_perm[0]), get_subgraph_cluster_nodes(timeslice_y, cluster_perm[1])
    ...
    while len(sorted_perm_dists) > 0:
        ...
        for i, perm_dist in enumerate(sorted_perm_dists):
            if perm_dist[0][0] in clusters_to_remove or perm_dist[0][1] in clusters_to_remove:
