...
class TimesliceClusterNetwork(FlowSpec):
    ...
    n_top = Parameter(
        "n_top",
        help="top n tf-idf words to name clusters.",
        type=int,
        default=3
        )
    ...
    def start(self):
        ...
        print(f"successfully loaded library data from {'s3://' + BUCKET_NAME + '/outputs/' + self.library_network_name}")
    ...
    def cluster_timeslice_network(self): 
        ...
        self.timeslices = timeslice_subject_pair_coo_graph(self.library_network, self.timeslice_interval, self.min_timeslice)
        self.subgraph_communities = cluster_timeslice_subject_pair_coo_graph(self.timeslices)
    ...
    def sanitise_clusters(self):
        ...
        for i in range(len(self.subgraph_communities) - 1):
            ...
            sanitise_clusters(
                self.subgraph_communities[timeslice_x], self.subgraph_communities[timeslice_y]
            )
    ...
    def color_name_clusters(self):
        ...
        self.subgraph_communities = add_cluster_names(self.subgraph_communities, self.n_top)
    ...
    def end(self):
        ...
        print(f"successfully saved library data to {'s3://' + BUCKET_NAME + '/outputs/' + self.library_timeslices}")
