...
def build_subject_matrix(mission_data, keyword):
    ...
    """
    #clean subjects 
    clean_subjects = clean_subject([book['bibliographic_data']['subject'] 
                for book in mission_data if 'subject' in book['bibliographic_data'].keys()])
    
    #remove searched keyword(s) 
    if isinstance(keyword, str):
        clean_subjects = [[sub for sub in subject if sub != keyword] for subject in clean_subjects if subject]

    elif isinstance(keyword, list):
        clean_subjects = [[sub for sub in subject if sub not in keyword] for subject in clean_subjects if subject]
    
    
    #build graph
    clean_subjects = [sub for sub in clean_subjects if sub != []]
    weighted_co_matrix = Counter(itertools.chain.from_iterable(itertools.combinations(line, 2) for line in clean_subjects))
...
def network_eda(network):
    ...
    print(f" there are {len(list(nx.selfloop_edges(network, data=True)))} self loops in the network.")

    centrality = nx.eigenvector_centrality(network)
    print(f"the graph's average eigenvector centrality value is {statistics.mean(list(centrality.values()))}.")
    print(f"based on eigenvector centrality, the most central nodes are: {[node for node, centrality_value in centrality.items() if centrality_value in sorted(list(centrality.values()), reverse=True)[:5]]}.")
    print('---------------')
...
dfcommunities = pd.DataFrame([(x[0], x[1]['cluster_group']) for x in list(G_climate.nodes.data())])
dfcommunities.columns = ['subject','cluster_group']

print(f"there are {len(list(set(dfcommunities['cluster_group'].tolist())))} clusters.")
print(f"the modularity of the clusters is {community_louvain.modularity(clusters, G_climate)}")
...
def expand_node_queries(G, std1 = 1.5, std2 = 2.5):
    ...
    two_stdevs = statistics.mean(ordered_centrality_scores) + std1*statistics.stdev(ordered_centrality_scores)
    three_stdevs = statistics.mean(ordered_centrality_scores) + std2*statistics.stdev(ordered_centrality_scores)
    ...
    most_central_scores = [most_central_score for most_central_score in 
                       ordered_centrality_scores if two_stdevs <= most_central_score <= three_stdevs]
...
print(len(list(set(list(G_climate.nodes())).intersection(list(expanded_books_graph.nodes())))))
